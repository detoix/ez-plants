...
| 7 — self-contained folder | **Held.** No plant imports another — the shared calendar moved to `src/lib/calendar.js`, ending the `dayOfYear` leak. Each shrub carries its own `leaf.webp` and loads it itself; bark is generated in `src/lib/bark-plate.js` and shared, since no plant owns it. `npm run plant:add` copies a plant that renders textured standing alone, with `three` as its only dependency. One gap remains: three of four models put `THREE.Vector3`/`Euler` into their snapshot instead of staying plain data. |
...
The open work: take Three.js back out of the model layer, give rules 4 and 5
the records and budget tests they ask for.
