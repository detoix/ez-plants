...
test('the sky defaults leave the shipped sun exactly where it was', () => {
  ...
  assert.equal(readFieldOptions('?sky=flat').sky, 'flat');
  assert.equal(readFieldOptions('?sky=anything').sky, 'atmosphere');

  const moved = readFieldOptions('?sunelevation=12&sunazimuth=-100&skyms=0');
...
test('the sky exposure is clamped rather than allowed to black the page out', () => {
  assert.ok(readFieldOptions('?skyexposure=0').skyExposure >= 0.5);
  assert.ok(readFieldOptions('?skyexposure=1000').skyExposure <= 60);
  assert.equal(
